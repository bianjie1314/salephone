...
<div class="page-container">
	<form action="" method="post" class="form form-horizontal" id="form-article-add">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" name="type" id="type" placeholder="" value="${phoneInfo.type}" class="input-text">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">原价：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" name="unitPrice" id="unitPrice" placeholder="" value="${phoneInfo.unitPrice}" class="input-text" style="width:90%">
				元</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">销售价：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" name="salePrice" id="salePrice" placeholder="" value="${phoneInfo.salePrice}" class="input-text" style="width:90%">
				元</div>
		</div>
		<div class="row cl">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<textarea name="description" cols="" rows="" class="textarea"  placeholder="说点什么...最少输入10个字符" datatype="*10-100" dragonfly="true" nullmsg="备注不能为空！" onKeyUp="$.Huitextarealength(this,200)">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" value="<fmt:formatDate value="${phoneInfo.startTime}" pattern="yyyy-MM-dd HH:mm:ss" />" onfocus="WdatePicker({ dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'#F{$dp.$D(\'datemax\')||\'%y-%M-%d\'}' })" id="datemin" class="input-text Wdate" name="startTime" style="width:180px;">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">销售结束时间：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" value="<fmt:formatDate value="${phoneInfo.endTime}" pattern="yyyy-MM-dd HH:mm:ss" />" onfocus="WdatePicker({ dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'#F{$dp.$D(\'datemin\')}' })" id="datemax" class="input-text Wdate" name="endTime"  style="width:180px;">
			</div>
		</div>
		<div class="row cl">
		...
		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
				<button onClick="formSubmit();" class="btn btn-primary radius" type="button"><i class="Hui-iconfont">&#xe632;</i> 提交</button>
...
function formSubmit(){
    if ($("[name='shopId']").val() == null || $("[name='shopId']").val() == "0"){
        ...
        layer.msg("输入商品型号",{icon:5,time:3000});
        return;
    } else if($("[name='unitPrice']").val() == null || $("[name='unitPrice']").val() == ""){
        layer.msg("请输入原价",{icon:5,time:3000});
        return;
    }else if($("[name='salePrice']").val() == null || $("[name='salePrice']").val() == ""){
        layer.msg("请输入销售价",{icon:5,time:3000});
        return;
    } else if($("[name='storeNum']").val() == null || $("[name='storeNum']").val() == ""){
        layer.msg("请输入库存数量",{icon:5,time:3000});
        return;
    } else if($("[name='startTime']").val() == null || $("[name='startTime']").val() == ""){
        layer.msg("请选择销售开始时间",{icon:5,time:3000});
        return;
    }else if($("[name='endTime']").val() == null || $("[name='endTime']").val() == ""){
        layer.msg("请选择销售结束时间",{icon:5,time:3000});
        return;
    }else {
        layer.confirm('确认修改吗？',function(index){
            $.ajax({
                type: 'post',
                url: '${pageContext.request.contextPath }/phone/update',
                dataType: 'json',
                ...
                data:JSON.stringify({
                    ...
                    "storeNum":$("[name='storeNum']").val(),
                    "description":$("[name='description']").val(),
                    "startTime":Date.parse(new Date($("[name='startTime']").val())),
                    "endTime":Date.parse(new Date($("[name='endTime']").val())),
                    "htmlText":$("[name='editorValue']").val()
                }),
                success: function(data){
                    if (data.result) {
